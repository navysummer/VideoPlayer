...
fn build(sysroot: Option<&str>) -> io::Result<()> {
    ...
    // TLS: enable a system TLS backend per target so https:// streams work
    // macOS/iOS use Apple's built-in SecureTransport (zero external deps)
    if matches!(target_os.as_str(), "ios" | "macos") {
        configure.arg("--enable-securetransport");
    }
    // Windows uses the built-in Schannel
    if matches!(target_os.as_str(), "windows") {
        configure.arg("--enable-schannel");
    }
